...
\path (o2) edge[inf, bend left=30] node {} (s2);
\end{tikzpicture}}
\caption{循环状态空间模型（RSSM）}
\label{fig:rssm}
\end{subfigure}%
\hfil\hfil%
\caption{潜在动力学模型设计。在该示例中，模型观测前两个时间步并预测第三个时间步。圆形表示随机变量，方形表示确定性变量；实线表示生成过程，虚线表示推断模型。(a) 循环神经网络中的转移完全是确定性的，因此模型难以捕捉多个可能未来，规划器也更容易利用模型误差。(b) 状态空间模型中的转移完全是随机的，因此难以在多个时间步中记住信息。(c) 我们将状态划分为随机部分和确定性部分，使模型能够更稳健地学习预测多种未来。}
\label{fig:models}
\end{figure*}
